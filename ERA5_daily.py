...
class ERA5_hourly_Precip:
    ...
    def __init__(self):
        self.this_class_arr, self.this_class_tif, self.this_class_png = T.mk_class_dir(
            ...
            this_script_root, mode=2)
        # self.product = 'total_precipitation'
        self.product = 'total_precipitation_hourly'

    def run(self):
        ee.Initialize()
        date_range_list = self.gen_date_list(start_date = '1982-01-01',end_date = '2021-01-01')
        outdir = join(self.this_class_arr,self.product)
    ...
    def download_images(self,startDate,endDate,outdir):
        start_year = startDate.split('-')[0]
        outdir = join(outdir,start_year)

        T.mk_dir(outdir,force=True)
        # print(startDate)
        # exit()

        out_path = join(outdir, f'{startDate.replace("-","")}.zip')
        # startDate = f'{year}-01-01'
        # endDate = f'{year+1}-01-01'
        # startDate = '2011-04-03'
        # endDate = '2011-04-04'
        ...
        try:
            self.download_i(url_sum, out_path)
        except:
            print('download error', out_path)
